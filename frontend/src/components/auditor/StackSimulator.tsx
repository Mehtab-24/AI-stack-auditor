...
export function StackSimulator({ auditResult }: StackSimulatorProps) {
  ...
  const originalCost = activeTools.reduce(
    (s: number, t: any) => s + (t.monthlyCost || t.monthly_cost || 0),
    0,
  );
  ...
  const runLocalSimulation = (toolsList: any[], text: string) => {
    ...
    if (
      ...
      // Look up tool
      const foundTool = toolsList.find(
        (t) =>
    ...
    if (
      ...
      const pct = match ? parseInt(match[1]) / 100 : 0.2;
    ...
    setResult({
      ...
      risk_score: 2.0,
      recommendation:
        "Try phrasing your test like: 'replace Cursor with Copilot', 'cancel Jasper', or 'reduce spend by 25%'.",
    });
  };
            ...
            <form onSubmit={handleSimulate} className="space-y-4">
              ...
              <div className="nav-tab-border">
                <button
                  ...
                  className="nav-tab-content w-full px-4 py-3 transition cursor-pointer nav-tab-active flex items-center justify-center gap-2 text-sm font-semibold text-black"
                  style={{
                    fontFamily: "'Product Sans', sans-serif",
                    fontWeight: 400,
                  }}
            ...
            <AnimatePresence mode="wait">
              {!result ? (
                ...
                >
                  ...
                  >
                    Describe a hypothetical change on the left and run the simulation to see
                    projected costs, savings, and organizational risk.
                  </p>
                </motion.div>
              ) : (
                /* RESULT DISPLAY */
                  ...
                  <div className="rounded-3xl border border-border bg-card p-6 shadow-sm space-y-5">
                    ...
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          className={`rounded-xl border px-3 py-1 text-xs font-bold ${getRiskColor(result.risk_score)} bg-opacity-5`}
                        >
